...
float AngleDiff::DiffAt(const PixelCoord& from, const PixelCoord& to) const {
  ...
  if (alpha > _params->horizontal_span() - 0.05) {
    // we are over the border
    const float span = _params->horizontal_span();
    if (alpha > span) {
      alpha -= span;
    } else {
      alpha = span - alpha;
...
void AngleDiff::PreComputeAlphaVecs() {
  ...
  for (size_t r = 0; r < _params->rows() - 1; ++r) {
    _row_alphas.push_back(
        fabs(_params->get_angle_from_row(r + 1) - _params->get_angle_from_row(r)));
  }
  // add last row alpha
  ...
  for (size_t c = 0; c < _params->cols() - 1; ++c) {
    _col_alphas.push_back(
        fabs( _params->get_angle_from_col(c + 1) - _params->get_angle_from_col(c) ) );
  }
  // handle last angle where we wrap columns
  float last_alpha = fabs( _params->get_angle_from_col(0) -
                           _params->get_angle_from_col(_params->cols() - 1) );
  last_alpha -= _params->horizontal_span();
...
cv::Mat AngleDiffPrecomputed::Visualize() const {
  ...
  for (int r = 0; r < _beta_rows.rows; ++r) {
    for (int c = 0; c < _beta_rows.cols; ++c) {
      ...
      auto row_angle = float(_beta_rows.at<float>(r, c)); // degree
      auto col_angle = float(_beta_cols.at<float>(r, c)); // degree
      uint8_t row_color = 255 * (row_angle / max_angle_deg);
      uint8_t col_color = 255 * (col_angle / max_angle_deg);
